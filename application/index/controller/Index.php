...
class Index extends Base
{
    ...
    {
        if (request()->isAjax()) {
            $data = ['name' => input('post.name'), 'password' => input('post.password'), 'conpass' => input('post.conpass'), 'nickname' => input('post.nickname'), 'email' => input('post.email'), 'verify' => input('post.verify')];
            $result = model('Member')->register($data);
            if ($result == 1) {
                $this->success('管理员添加成功', 'index/index/login');
            } else {
                $this->error($result);
            }
        }
    ...
    {
        if (request()->isAjax()) {
            $data = ['name' => input('post.name'), 'password' => input('post.password'), 'verify' => input('post.verify')];
